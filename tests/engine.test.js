...
for(const a of D.ACTIVE_CLASSES)for(const b of D.ACTIVE_CLASSES)for(const level of [1,10,20]){
...
for(const cls of D.ACTIVE_CLASSES)for(let id=0;id<D.ITEMS.length;id++){const h=hero(20,cls);h.inventory=[{id,level:20}];h.equipped=[-1,-1,-1,-1];h.equipped[D.ITEMS[id].slot]=0;let f=fighter(h);assert(f.maxhp>0&&f.speed>0);assert(Number.isFinite(value(h.inventory[0])));}
for(const kind of ['skill','hit','burn','shield','dodge','heal','wolf','copy','turret'])assert(kinds.has(kind),kind+' observed');
