...
const art=prepareRetro(body,mage,heads,canvas(1536,1024),(w,h)=>{created++;return canvas(w,h);},body,body,body,body,koHeads,body);
const ctx=canvas(800,600).getContext('2d');
let count=0;
for(const cls of [0,1,2,4,6,10,12])for(let identity=0;identity<24;identity++)for(let armor=6;armor<=9;armor++)for(let weapon=-1;weapon<6;weapon++)for(let pose=0;pose<6;pose++)for(const flip of [false,true]){
...
console.log(`${count} retro renders: all Warrior, Archer, Mage, Knight, Ninja, Necromancer and Boxer identities, equipment, poses, directions, legacy saves and shared cache verified.`);
...
for(const cls of [0,1,2,4,6,10,12]){
 const art=prepareRetro(body,mage,heads,canvas(1536,1024),canvas,body,body,body,body,koHeads,body);
