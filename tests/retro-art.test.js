...
const art=prepareRetro(body,mage,heads,canvas(1536,1024),(w,h)=>{created++;return canvas(w,h);});
const ctx=canvas(800,600).getContext('2d');
let count=0;
for(const cls of [0,2])for(let identity=0;identity<24;identity++)for(let armor=6;armor<=9;armor++)for(let weapon=-1;weapon<6;weapon++)for(let pose=0;pose<6;pose++)for(const flip of [false,true]){
...
console.log(`${count} retro renders: all Warrior and Mage identities, both classes, equipment, poses, directions, legacy saves and shared cache verified.`);
