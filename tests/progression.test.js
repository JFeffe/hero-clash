...
let h=hero(1,0);h.inventory=[{id:0,level:1}];h.equipped=[0,-1,-1,-1];const common=fighter(h);h.inventory[0].rarity=2;let rare=fighter(h);assert(Math.abs(value(h.inventory[0])-D.ITEMS[0].base*1.5)<1e-10);assert(rare.physical>common.physical);assert.equal(rare.accuracy,common.accuracy);assert.equal(rare.speed,common.speed);
