...
let h=hero();h.inventory=[{id:0,level:1}];h.equipped=[0,-1,-1,-1];const common=fighter(h);h.inventory[0].rarity=2;let rare=fighter(h);assert(Math.abs(value(h.inventory[0])-.15)<1e-10);assert(rare.physical>common.physical);assert.equal(rare.accuracy,.03);assert(Math.abs(rare.speed-common.speed-.05)<1e-10);
