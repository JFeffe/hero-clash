...
root=pathlib.Path(__file__).resolve().parents[2];out=root/'balance/synergy-study'
def execute(mode):
 with tempfile.TemporaryDirectory(prefix='hero-synergy-') as tmp:
  ...
  if mode in ['burn','both']:
   ...
   new='Math.max(q.burn>0?q.burnPower:0,p.ignite+(skill&&sc===2?4:0))'
  ...
  if mode in ['dog','both']:
   ...
   assert s.count(old)==1;s=s.replace(old,new)
  f.write_text(s)
  f=w/'balance/matrix-audit/run.mjs';s=f.read_text().replace('levels=[1,5,10,15,20],n=300,baseSeed=130920261','levels=[1,10,20],n=150,baseSeed=2713092026');f.write_text(s)
  ...
  for tag,classes,profile,gear in [('all','0,1,2,3,4,6,7,8,9','balanced','automatic'),('mage','2','resilient','smart')]:
   ...
   subprocess.run(['node','balance/decision-audit/run.mjs',f'--classes={classes}',f'--profile={profile}',f'--gear={gear}','--per-class=150','--seed=2813092026',f'--out={dest}'],cwd=w,check=True,stdout=subprocess.DEVNULL)
