...
const n=Number(arg('per-class',2000)),baseSeed=Number(arg('seed',20260913)),out=path.resolve(arg('out','balance/career-audit/results.json'));
assert(Number.isInteger(n)&&n>0);assert(Number.isInteger(baseSeed));
const policies=arg('policies','random,cautious').split(',');const classes=arg('classes',D.ACTIVE_CLASSES.join(',')).split(',').map(Number);assert(policies.every(p=>['random','cautious'].includes(p)));assert(classes.every(c=>D.ACTIVE_CLASSES.includes(c)));const mergeDir=arg('merge','');
...
// Match applyCopy's policy: offensive stat, first item offer, then equip loot.
// This is a reproducible reference, not an optimized human equipment strategy.
function rewards(h,loot){
 for(const p of h.pending){if(p.type==='stat')h.stats[D.MAGIC_CLASSES.includes(h.class)?2:0]++;else if(p.offers?.length){const it=p.offers[0];assert.notEqual(acquireItem(h,it),false);assert(equipItem(h,D.ITEMS[it.id].slot,h.inventory.length-1));}}
 h.pending=[];
 if(loot)assert(equipItem(h,D.ITEMS[loot.id].slot,h.inventory.indexOf(loot)));
}
function career(cls,policy,seed){
 ...
 try{
  ...
  while(g.heroes.includes(h)){
   ...
   if(g.heroes.includes(h))rewards(h,result.loot);
...
const all=[],summary={version:JSON.parse(fs.readFileSync('package.json')).version,sourceCommit:execFileSync('git',['rev-parse','HEAD'],{encoding:'utf8'}).trim(),baseSeed,perClassPerPolicy:n,policies:{},method:{energy:'Wait abstracted; 20 consumed per fight',opponents:'Production 9-member pool, 3 offers, maximum 2 meetings; copies fight every round',rewards:'Main offensive stat; first offered item; equip new reward then loot, like production bot copies',rerolls:'None, including the free opponent reroll',retirement:'Survive ten matches on floor 15; ten wins not required',seed:'LCG32 per career: baseSeed + class*1000003 + index*7919; same initial seed across policies; UUID/time do not affect outcomes'},validation:{completed:true,invariants:'Energy, legal opponents, maximum meetings, hearts before climb, recovery, floor cap, 150-match termination bound, terminal state, totals'}};
// Reproducibility check excludes UUIDs/timestamps by retaining only outcome data.
assert.deepEqual(career(0,'random',baseSeed),career(0,'random',baseSeed));
...
summary.totalCareers=all.length;summary.totalPlayerMatches=all.reduce((s,r)=>s+r.fights,0);summary.totalPoolMatches=summary.totalPlayerMatches*5;
