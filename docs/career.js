...
export function createPool(h){
 ...
 return {members,remaining:shuffle([...rounds,...structuredClone(rounds)]),played:0,meetings:Array(9).fill(0),lastRound:[],results:[]};
}
...
export function resolvePool(h,opponent,result){
 const p=h.career.pool;p.results??=[];const index=p.members.findIndex(m=>m.id===opponent.id)+1;
 ...
 }p.results.push({opponent:opponent.name,winner:result.winner});p.meetings[index-1]++;p.played++;
}
export function advanceCareer(h){
 const c=h.career,p=c.pool;if(p.played<10)return null;
 const from=c.floor,remainingHearts=h.hearts,dead=h.hearts===0,retired=!dead&&from===15;
 const summary={from,floor:dead||retired?from:Math.min(15,from+remainingHearts),remainingHearts,hearts:dead||retired?h.hearts:Math.min(5,h.hearts+1),dead,retired,results:structuredClone(p.results??[])};
 for(const [key,winner] of [['wins',0],['losses',1],['draws',-1]])summary[key]=summary.results.filter(r=>r.winner===winner).length;
 c.history.push({floor:from,hearts:remainingHearts,rounds:10,members:structuredClone(p.members),summary});
 if(dead)return {summary};
 if(retired){h.retired_at=Date.now();return {retired:true,floor:15,summary};}
 c.floor=summary.floor;h.hearts=summary.hearts;c.pool=createPool(h);
 return {from,floor:c.floor,hearts:h.hearts,summary};
}
