import {FAMILIARS,prepareFamiliars} from './familiar-art.js?v=0.19.0';
import {prepareRetro} from './retro-art.js?v=0.19.0';
import {prepareWarriorBodies} from './warrior-identity.js';
import {appearanceOf,validAppearance} from './appearance.js';
import {drawWarriorGear} from './warrior-gear.js?v=0.19.0';
import {cleanSpriteMatte,cleanGreenMatte} from './sprite-matte.js?v=0.19.0';
...
export function loadAtlas(){
 ...
 pending=Promise.all([loadSheet('./assets/warrior-wolf-v1.webp',cleanSpriteMatte),loadSheet('./assets/mage-v1.webp',cleanGreenMatte),loadSheet('./assets/warrior-armors-v1.webp',cleanSpriteMatte),loadSheet('./assets/warrior-weapons-v1.webp',cleanSpriteMatte),loadSheet('./assets/warrior-heads-v1.png',cleanGreenMatte),loadSheet('./assets/warrior-retro-v1.png',cleanGreenMatte),loadSheet('./assets/mage-retro-v1.png',cleanGreenMatte),loadSheet('./assets/heads-retro-v1.png',cleanGreenMatte),loadSheet('./assets/archer-retro-v1.png',cleanSpriteMatte),loadSheet('./assets/necro-retro-v1.png',cleanGreenMatte),loadSheet('./assets/boxer-retro-v1.png',cleanGreenMatte),loadSheet('./assets/ninja-retro-v1.png',cleanGreenMatte),loadSheet('./assets/heads-ko-v1.png',cleanGreenMatte),loadSheet('./assets/knight-retro-v1.png',cleanGreenMatte),loadSheet('./assets/knight-shield-v1.png',cleanGreenMatte),...FAMILIARS.map(name=>loadSheet(`./assets/${name}-retro-v1.png`,name==='cat'||name==='raven'?cleanGreenMatte:()=>{}))]).then(([w,m,b,g,heads,rw,rm,rh,ra,rn,rb,ri,rk,rt,rs,...pets])=>{try{familiars=prepareFamiliars(pets);}catch(error){console.warn("Familiar art unavailable",error);}if(rw&&rm&&rh&&g){try{retro=prepareRetro(rw,rm,rh,g,(w,h)=>{const c=document.createElement('canvas');c.width=w;c.height=h;return c;},ra,rn,rb,ri,rk,rt,rs);}catch(error){console.warn('Retro art unavailable',error);}}sheet=w;mageSheet=m;warriorBody=b;warriorWeapons=g;warriorHeads=heads;if(b&&heads){try{warriorBodies=prepareWarriorBodies(b,(w,h)=>{const c=document.createElement('canvas');c.width=w;c.height=h;return c;});}catch(error){console.warn('Warrior identity unavailable',error);}}return Boolean(retro||w||m||b);});return pending;
}
