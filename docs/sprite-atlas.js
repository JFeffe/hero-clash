import {drawMageGear} from './mage-gear.js?v=0.10.0';
import {drawWarriorGear} from './warrior-gear.js?v=0.10.0';
import {cleanSpriteMatte,cleanGreenMatte} from './sprite-matte.js?v=0.10.0';
let sheet=null,mageSheet=null,warriorBody=null,warriorWeapons=null,mageBody=null,pending;
export const atlasReady=()=>Boolean(sheet||mageSheet||mageBody);
...
export function atlasWarrior(ctx,h,x,y,s,flip,time,fallen,pose){
 if(h.class===0&&warriorBody&&warriorWeapons){drawWarriorGear(ctx,warriorBody,warriorWeapons,h,animationFrame(time,pose,fallen),x,y+27*s-(s*27-24)*(Number(fallen)||0),s,flip,time);return true;}
...
export function atlasMage(ctx,h,x,y,s,flip,time,fallen,pose){
 if(h.class===2&&mageBody&&warriorWeapons){drawMageGear(ctx,mageBody,warriorWeapons,h,animationFrame(time,pose,fallen),x,y+27*s-(s*27-24)*(Number(fallen)||0),s,flip,time);return true;}
